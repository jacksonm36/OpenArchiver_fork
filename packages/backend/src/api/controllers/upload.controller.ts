...
import { assertAllowedUploadedFilePath } from '../../helpers/localImportPath';
...
export const uploadFile = async (req: Request, res: Response) => {
	...
	bb.on('file', (fieldname, file, info) => {
		...
		try {
			assertAllowedUploadedFilePath(filePath);
		} catch {
			file.resume();
			req.unpipe(bb);
			bb.removeAllListeners();
			sendErrorResponse(400, 'Invalid upload destination path.');
			return;
		}
